...
const getData = (keys: string[], input: Record<string, any>) => {
...
export const sanitize = (req: Request, _: Response, next: NextFunction) => {
	...
	if (queryKeys.length > 0) {
		req.query = getData(queryKeys, req.query);
	}
	if (paramKeys.length > 0) {
		req.params = getData(paramKeys, req.params);
	}
	if (bodyKeys.length > 0) {
		req.body = getData(bodyKeys, req.body);
	}
	next();
};
