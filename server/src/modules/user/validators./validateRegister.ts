  ...
  if (!firstName.match(firstNameRegExp)) {
    errors = [
      ...errors,
      {
        field: "firstName",
        message:
          "First name only contains letters and appropriate symbols dash(-) or aphostrope('). Space is not allowed",
      },
    ];
  }
  if (lastName && !lastName.match(lastNameRegExp)) {
    errors = [
      ...errors,
      {
        field: "lastName",
        message:
          "Last name can contains letters, appropriate symbols dash(-) or aphostrope('), and space",
      },
    ];
  }
